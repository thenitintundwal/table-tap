        ...
        <div className="fixed inset-y-0 right-0 w-full max-w-md bg-white dark:bg-zinc-950 border-l border-zinc-200 dark:border-white/10 z-[110] shadow-2xl animate-in slide-in-from-right duration-300">
            ...
            <div className="p-6 border-b border-zinc-100 dark:border-white/5 flex items-center justify-between bg-zinc-50 dark:bg-zinc-900/50">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-xl font-black text-zinc-900 dark:text-white">{customer.customer_name}</h2>
                        <p className="text-zinc-500 dark:text-zinc-400 text-[10px] font-black uppercase tracking-[.2em] flex items-center gap-2">
                            <Clock className="w-3.5 h-3.5 text-orange-600 dark:text-orange-500" />
                ...
                <button
                    onClick={onClose}
                    className="p-2 hover:bg-zinc-200 dark:hover:bg-white/5 rounded-xl text-zinc-500 hover:text-zinc-900 dark:hover:text-white transition-all"
                >
                    <X className="w-6 h-6" />
                </button>
            </div>
            ...
            <div className="p-6 space-y-8 overflow-y-auto h-[calc(100vh-100px)] custom-scrollbar">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-100 dark:border-white/5 p-4 rounded-2xl">
                        ...
                        <p className="text-2xl font-black text-zinc-900 dark:text-white">₹{customer.total_spend.toLocaleString()}</p>
                    </div>
                    <div className="bg-zinc-50 dark:bg-card border border-zinc-100 dark:border-border p-4 rounded-2xl">
                    ...
                    <div className="bg-zinc-50 dark:bg-card border border-zinc-100 dark:border-border p-4 rounded-2xl">
                        ...
                        <p className="text-2xl font-black text-zinc-900 dark:text-white">{customer.visit_count} visits</p>
                    </div>
                    <div className="bg-zinc-50 dark:bg-card border border-zinc-100 dark:border-border p-4 rounded-2xl">
                            ...
                            history.map((tx) => (
                                <div key={tx.id} className="bg-zinc-50 dark:bg-white/5 border border-zinc-100 dark:border-white/5 p-4 rounded-2xl flex items-center justify-between group hover:border-orange-500/20 transition-all">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-sm font-bold text-zinc-900 dark:text-white">{tx.description || (tx.type === 'earn' ? 'Manual Reward' : 'Redemption')}</p>
                ...
                <div className="pt-6 border-t border-zinc-100 dark:border-white/5">
                    <p className="text-[10px] font-black text-zinc-500 dark:text-zinc-600 uppercase tracking-widest mb-3">Quick Actions</p>
                    <button
                        disabled
                        className="w-full bg-zinc-100 dark:bg-zinc-900 text-zinc-400 dark:text-zinc-600 border border-zinc-200 dark:border-white/5 rounded-xl py-4 text-[10px] font-black transition-all opacity-50 cursor-not-allowed uppercase tracking-widest"
                    >
